import { NextResponse } from "next/server";
import { createSupabaseAdminClient, createSupabaseUserClient } from "@/lib/supabase/server";
import { tapeSideAt } from "@/shared/floor-map";

type TapeJoinBody = {
  roundId?: unknown;
  stake?: unknown;
  idempotencyKey?: unknown;
};

function isUuid(value: string) {
  return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value);
}

async function getUser(request: Request) {
  const authorization = request.headers.get("authorization");
  const accessToken = authorization?.startsWith("Bearer ") ? authorization.slice("Bearer ".length) : null;

  if (!accessToken) return { userId: null, error: "Missing Supabase session." };

  const userSupabase = createSupabaseUserClient(accessToken);
  const {
    data: { user },
    error
  } = await userSupabase.auth.getUser(accessToken);

  if (error || !user) return { userId: null, error: "Invalid Supabase session." };
  return { userId: user.id, error: null };
}

export async function POST(request: Request) {
  const { userId, error: userError } = await getUser(request);

  if (!userId) {
    return NextResponse.json({ error: userError }, { status: 401 });
  }

  let body: TapeJoinBody;
  try {
    body = (await request.json()) as TapeJoinBody;
  } catch {
    return NextResponse.json({ error: "Invalid JSON body." }, { status: 400 });
  }

  if (
    typeof body.roundId !== "string" ||
    !isUuid(body.roundId) ||
    typeof body.stake !== "number" ||
    !Number.isSafeInteger(body.stake) ||
    body.stake <= 0 ||
    typeof body.idempotencyKey !== "string" ||
    body.idempotencyKey.trim().length < 8 ||
    body.idempotencyKey.length > 120
  ) {
    return NextResponse.json({ error: "Invalid Tape stake payload." }, { status: 400 });
  }

  const supabase = createSupabaseAdminClient();
  const { data: player, error: playerError } = await supabase
    .from("players")
    .select("id, gx, gy")
    .eq("id", userId)
    .maybeSingle();

  if (playerError) {
    return NextResponse.json({ error: playerError.message }, { status: 400 });
  }

  if (!player) {
    return NextResponse.json({ error: "Player not found." }, { status: 404 });
  }

  const side = tapeSideAt(player.gx, player.gy);
  if (!side) {
    return NextResponse.json({ error: "Walk onto the UP or DOWN pad before staking." }, { status: 400 });
  }

  const { data, error } = await supabase.rpc("join_tape_round", {
    p_player_id: userId,
    p_round_id: body.roundId,
    p_side: side,
    p_stake: body.stake,
    p_idempotency_key: body.idempotencyKey
  });

  if (error) {
    return NextResponse.json({ error: error.message }, { status: 400 });
  }

  const stake = Array.isArray(data) ? data[0] : data;
  return NextResponse.json({ stake });
}
