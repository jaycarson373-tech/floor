...
export async function POST(request: Request) {
  ...
  let body: DealJoinBody;
  try {
    body = (await request.json()) as DealJoinBody;
  } catch {
  ...
  if (
    ...
    return NextResponse.json({ error: "Invalid Deal entry payload." }, { status: 400 });
  }

  const supabase = createSupabaseAdminClient();
  const { data, error } = await supabase.rpc("join_deal", {
    p_player_id: userId,
  ...
  return NextResponse.json({ entry: Array.isArray(data) ? data[0] : data });
}
